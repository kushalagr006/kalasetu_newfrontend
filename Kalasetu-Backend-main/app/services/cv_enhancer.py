...
def get_rembg_session():
    ...
    if REMBG_SESSION is None:
        print("[INFO] Pre-loading rembg ISNet AI model session (isnet-general-use)...")
        try:
...
def get_sr_model():
    ...
    if SR_MODEL is None:
        model_paths = [
            "FSRCNN_x4.pb",
            os.path.join(os.path.dirname(__file__), "..", "..", "FSRCNN_x4.pb"),
            os.path.join(os.getcwd(), "FSRCNN_x4.pb"),
        ]
        found_path = None
...
def enhance_product_image(original: Image.Image, canvas_size=(1400, 1400)) -> Image.Image:
    ...
    if sr_impl is not None:
        try:
            ...
            hd_product = corrected.resize((corrected.width * 4, corrected.height * 4), Image.Resampling.LANCZOS)
    else:
        hd_product = corrected.resize((corrected.width * 4, corrected.height * 4), Image.Resampling.LANCZOS)
...
def enhance_product_bytes(image_bytes: bytes) -> bytes:
    try:
